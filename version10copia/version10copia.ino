...
void calcular_momentos_hu_corregido(uint8_t* imagen_binaria, int width, int height, double hu[7], double* cx_out = nullptr, double* cy_out = nullptr) {
    ...
    // Guardar centroide para uso externo
    if (cx_out) *cx_out = cx;
    if (cy_out) *cy_out = cy;
...
void detectar_figura_mejorada(uint8_t* imagen_binaria, int width, int height, char* figura, double* confianza) {
    ...
    if (mejor_idx != -1 && mejor_sim >= umbral_adaptativo) {
        ...
        *confianza = mejor_sim;
        ...
        // Debug serial
        Serial.printf("✅ Detectado: %s (%.1f%%) - Centroide: (%.1f, %.1f) - Muestras: %d\n", 
                     figura, mejor_sim * 100, cx, cy, figuras_calibradas[mejor_idx].muestras);
    } else {
    ...
    while(true) {
        ...
        if (fb->format == PIXFORMAT_JPEG && buffers_initialized) {
            ...
            if (rgb565_buffer && jpg2rgb565(fb->buf, fb->len, rgb565_buffer, JPG_SCALE_NONE)) {
                ...
                // 🔥 PINTAR CENTROIDE EN ROJO si es válido
                if (ultimo_cx >= 0 && ultimo_cy >= 0) {
                    pintar_centroide_en_imagen(rgb_buffer, fb->width, fb->height, ultimo_cx, ultimo_cy);
                }
                ...
                if (fmt2jpg(rgb_buffer, grayscale_len * 3, fb->width, fb->height, 
                    ...
                    char metadata[150];
                    snprintf(metadata, sizeof(metadata), 
                            "\r\nX-Detection-Data: {\"figura\":\"%s\",\"confianza\":%.2f,\"centroide_x\":%.1f,\"centroide_y\":%.1f}\r\n", 
                            ultima_figura, ultima_confianza, ultimo_cx, ultimo_cy);
...
void setup() {
    ...
    Serial.println("║   CON CENTROIDE EN ROJO             ║");
    Serial.println("╚══════════════════════════════════════╝\n");
